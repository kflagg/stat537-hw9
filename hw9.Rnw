...
\item %b
The CFI is 0.953. This tells us that this model does a very good job of explaining the variation in the manifest variables, although the CFI contradicts the result of the goodness-of-fit test. It seems that, given the amount of information in the dataset, there is room for improvement in the model fit.
...
The correlation between the true preoccupied with food variable and the true health feelings variable is 0.65. It makes sense that the true preoccupied latent trait would be moderately positively correlated with the health feelings latent trait because the variables that loaded on the health feelings latent trait were {\texttt ow, avd, glt, occ, brn, fat}, and {\texttt emp}. These manifest variables also desribe traits of people that may be overly conscious of their weight.
...
Based on a z statistic of 1.564 and an associated pvalue of 0.118, there is little evidence that \verb|__| {\texttt bmi} is \verb|___| to the {\texttt preoccupied with food} latent variable.

Based on a z statistic of 3.347 and an associated pvalue of 0.001, there is strong evidence that \verb|__| {\texttt bmi} is \verb|___| to the {\texttt HealthFeelings} latent variable.
